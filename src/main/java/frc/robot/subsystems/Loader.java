...
public class Loader extends SubsystemBase {
    ...
    private void logTelemetry() {
        Logger.recordOutput("Loader/VelocityRPS",
            motor.getVelocity().getValueAsDouble());

        Logger.recordOutput("Loader/VelocityRPM",
            motor.getVelocity().getValueAsDouble() * 60.0);

        Logger.recordOutput("Loader/SupplyCurrent",
            motor.getSupplyCurrent().getValueAsDouble());

        Logger.recordOutput("Loader/StatorCurrent",
            motor.getStatorCurrent().getValueAsDouble());

        Logger.recordOutput("Loader/CommandedPercent",
            commandedPercent);

        Logger.recordOutput("Loader/Running",
            running);
    }
}