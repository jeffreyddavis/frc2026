...
public class Spindexer extends SubsystemBase {
    ...
    private void logTelemetry() {
        Logger.recordOutput("Spindexer/VelocityRPS",
            motor.getVelocity().getValueAsDouble());

        Logger.recordOutput("Spindexer/VelocityRPM",
            motor.getVelocity().getValueAsDouble() * 60.0);

        Logger.recordOutput("Spindexer/SupplyCurrent",
            motor.getSupplyCurrent().getValueAsDouble());

        Logger.recordOutput("Spindexer/StatorCurrent",
            motor.getStatorCurrent().getValueAsDouble());

        Logger.recordOutput("Spindexer/CommandedPercent",
            commandedPercent);

        Logger.recordOutput("Spindexer/Running",
            running);
    }
}