...
// =====================
    ...
    private final LoggedNetworkNumber rpmOffset =
        new LoggedNetworkNumber("Shot/RPMOffset", 0.0);
    ...
    public ShotSolution calculateStationary(
        Translation2d robotPosition,
        Translation2d goalPosition
    ) {
        ...
        boolean tuningAllowed =
            enableTuning.get() > 0.5 &&
    ...
    public ShotSolution calculateWithLead(
        ...
        Translation2d futurePos =
            robotPosition.plus(robotVelocity.times(latencySeconds));
        ...
        double baselineSpeed =
            (distance / baseline.timeOfFlightSeconds());

        Translation2d baselineVelocityVector =
            toGoal.div(distance).times(baselineSpeed);

        Translation2d shotVelocity =
            baselineVelocityVector.minus(robotVelocity);
    ...
    private ShotParams getParams(double distance) {
        ...
        double clamped =
            MathUtil.clamp(distance, minDistance, maxDistance);
